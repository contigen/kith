...
export function SimulationInterface({ agent }: { agent: Agent }) {
            ...
            <CardHeader className='bg-gradient-to-r from-primary/5 to-transparent'>
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <CardDescription>
                    Review the agent&apos;s credentials before proceeding with
                    the interaction
                  </CardDescription>
                </div>
                      ...
                      <ul className='mt-1 space-y-1 text-xs text-muted-foreground'>
                        {agent.credentials.map(credential => (
                          <li key={credential.type}>
                            {credential.type}:{' '}
                            {credential.verified
                              ? `Issued by ${'Kith'} on ${credential.createdAt.toLocaleDateString()}`
                              : 'Not verified'}
                          </li>
                        ))}
                      </ul>
